...
def create_dashboard(df, dollar_format_metrics, metric_definitions, start_date, end_date):
    ...
    sidebar = html.Div(
        [
            html.H4("U.S. Bank Metrics", className="display-6 mb-4", style={"color": "#333333"}),
    ...
    content = html.Div([
        dbc.Card([
            dbc.CardHeader([
                dbc.Row([
                    dbc.Col(html.H4("U.S. Bank vs Peer Banks", className="card-title"), width=8),
                    dbc.Col(
                        dcc.Dropdown(
                            id='date-selector',
    ...
    def update_bar_chart(selected_metric, selected_date, selected_peers):
        ...
        # Always include U.S. Bank, even if no peers are selected
        selected_banks = ['U.S. Bank'] + (selected_peers if selected_peers else [])
        ...
        colors = ['#00438F' if bank == 'U.S. Bank' else '#808080' for bank in sorted_df['Bank']]
        ...
        for bank in trend_df.columns:
            color = '#00438F' if bank == 'U.S. Bank' else '#808080'
            line_width = 3.5 if bank == 'U.S. Bank' else 1.5
            opacity = 1 if bank == 'U.S. Bank' else 0.4
        ...
        usb_value = filtered_df[filtered_df['Bank'] == 'U.S. Bank'][selected_metric].values[0] if 'U.S. Bank' in filtered_df['Bank'].values else None

        # Calculate additional statistics
        usb_percentile = stats.percentileofscore(filtered_df[selected_metric], usb_value) if usb_value is not None else None
        usb_rank = filtered_df[selected_metric].rank(ascending=False, method='min')[filtered_df['Bank'] == 'U.S. Bank'].values[0] if usb_value is not None else None
        q1, q3 = np.percentile(filtered_df[selected_metric], [25, 75])
        iqr = q3 - q1

        # Calculate trend statistics
        trend_df = df[df['Bank'].isin(selected_banks)].pivot(index='Date', columns='Bank', values=selected_metric)
        usb_trend = trend_df['U.S. Bank'].dropna() if 'U.S. Bank' in trend_df.columns else pd.Series()
        peer_trends = trend_df.drop('U.S. Bank', axis=1, errors='ignore').dropna()

        if not usb_trend.empty:
            usb_slope, _ = np.polyfit(range(len(usb_trend)), usb_trend, 1)
            usb_volatility = usb_trend.std()
        else:
            usb_slope = usb_volatility = np.nan
        ...
        if not peer_trends.empty:
            ...
            correlations = peer_trends.corrwith(usb_trend)
        else:
        ...
        overview = html.Div([
            ...
            html.Div([
                ...
                html.Div([
                    html.Div("U.S. Bank's Value:", className="stat-label usb-highlight"),
                    html.Div(format_value(usb_value) if usb_value is not None else "N/A", className="usb-highlight")
                ], className="stat-row"),
                html.Div([
            ...
            # U.S. Bank Snapshot Statistics Section
            html.Div([
                html.Div("U.S. Bank's Position", className="stat-section-title"),
                html.Div([
                    html.Div("Percentile Rank:", className="stat-label"),
                    html.Div(f"{usb_percentile:.1f}%" if usb_percentile is not None else "N/A")
                ], className="stat-row"),
                html.Div([
                    html.Div("U.S. Bank's Ranking:", className="stat-label"),
                    html.Div(f"#{usb_rank:.0f} out of {len(filtered_df)} banks" if usb_rank is not None else "N/A")
                ], className="stat-row"),
                html.Div([
                    html.Div("Performance Group:", className="stat-label"),
                    html.Div(f"{'Top 25%' if usb_value > q3 else 'Bottom 25%' if usb_value <= q1 else 'Middle 50%'}" if usb_value is not None else "N/A")
                ], className="stat-row"),
                html.Div([
                    html.Div("Standout Score:", className="stat-label"),
                    html.Div(f"{stats.zscore(filtered_df[selected_metric])[filtered_df['Bank'] == 'U.S. Bank'].values[0]:.2f} " + 
                             "(How many standard deviations from the average)" if usb_value is not None else "N/A")
                ], className="stat-row"),
            ], className="stat-section"),
            ...
            html.Div([
                ...
                html.Div([
                    html.Div("U.S. Bank's Growth Rate:", className="stat-label"),
                    html.Div(f"{usb_slope:.4f} per year" if not np.isnan(usb_slope) else "N/A")
                ], className="stat-row"),
                html.Div([
                ...
                html.Div([
                    html.Div("U.S. Bank's Stability:", className="stat-label"),
                    html.Div(f"{usb_volatility:.4f} (Lower means more stable)" if not np.isnan(usb_volatility) else "N/A")
                ], className="stat-row"),
                html.Div([
    ...
    def display_click_data(clickData, selected_date, selected_metric, selected_peers):
        ...
        # Always include U.S. Bank
        selected_banks = ['U.S. Bank'] + selected_peers
        ...
        return [
            html.H5(f"{bank}", style={'color': '#00438F' if bank == 'U.S. Bank' else '#333333'}),
